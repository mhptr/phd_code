...
# #write.csv(data.pr.imputed, "./Analysis - R-Script /Data/combined/output/imputated/data.pr.imputed.csv")
# annotation.all$T..ENSG
# data.pr.imputed = read.csv("./Analysis - R-Script /Data/combined/output/imputated/data.pr.imputed.csv")
# data.pr.imputed_annot = merge(data.pr.imputed, annotation.all, by.x="X",by.y="T..ENSG",all.x=TRUE)
# #write.csv(data.pr.imputed_annot, "./Analysis - R-Script /Data/combined/output/imputated/data.pr.imputed_annot.csv", row.names = F)
...
l <- layout_with_fr(g.LB)
plot(g.LB, layout = l, vertex.frame.color=NA,vertex.label.dist=0.5, edge.arrow.size=0.2, 
     mark.border="red",pch=19,vertex.label=degree.LB$Degree, #rownames(degree.LB[(degree.LB$Degree)>10 ,]) = gives BSU numbers in the name 
     xlim=c(-0.8,0.8), ylim = c(-0.5,1.5),
     #vertex.color=colorRampPalette(c("white", "red")) (10),
     vertex.color=degree.U$Degree,        #gives multiple colours 
...
regulations2.U = regulations2[regulations2$regulator.locus.tag%in%rownames(diffexp.pr.all.M9)[diffexp.pr.all.M9$adj.P.Val<0.05] | 
                                regulations2$locus.tag%in%rownames(diffexp.pr.all.M9)[diffexp.pr.all.M9$adj.P.Val<0.05], ]
#regulations2.U = regulations2 
g.U = graph.data.frame(cbind(as.character(regulations2.U$regulator.locus.tag),
                             as.character(regulations2.U$locus.tag)), directed=F)  #no direction decided at this step
g.U = igraph::simplify(g.U)  #sort of finds the unique interactions
...
regulations2.M9 = regulations2[regulations2$regulator.locus.tag%in%rownames(diffexp.pr.all.M9)[diffexp.pr.all.M9$adj.P.Val<0.05] | 
                                 regulations2$locus.tag%in%rownames(diffexp.pr.all.M9)[diffexp.pr.all.M9$adj.P.Val<0.05], ]
...
plot(g.M9, layout = l, vertex.frame.color=NA, vertex.label.dist=0.5, edge.arrow.size=0.2, 
     ...
     #vertex.color=dim(degree.U),  #gives two colours : orange and blue
     vertex.label.color="black", vertex.label.dist=1.5,
     #vertex.shape=diffexp.pr.all.M9_SIG$P.Value,
     vertex.shape="circle",  #how to set two shapes for two kinds of expression data
     vertex.size=degree.M9$Degree, 
     scale=degree.M9$Degree)    #check pch #[degree.M9$Degree<300 = 800,]
#how to make the size of each each in a certain margin length eg all value >20 =20
#title("Regulatory Networks in M9 - Proteomics",cex.main=1,col.main="black")
